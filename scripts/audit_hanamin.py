...
SOURCE_ROOT = ROOT / ".work" / "hanamin" / "source"
DIST_ROOT = ROOT / "packages" / "hanamin" / "dist"
FAMILIES = ("HanaMinA", "HanaMinB")
EXPECTED_CODEPOINT_SUM = 101_912
EXPECTED_CODEPOINT_UNION = 98_703
EXPECTED_VARIATION_SEQUENCES = 29_772
EXPECTED_WOFF2_FILES = 226
        ...
        for table in font["cmap"].tables:
            if table.format == 14:
                for selector, entries in table.uvsDict.items():
                    sequences.update((base, selector, glyph_name is None) for base, glyph_name in entries)
...
def main() -> None:
    ...
    for family in FAMILIES:
        family_source_codepoints, family_source_sequences = inspect_font(SOURCE_ROOT / f"{family}.ttf")
        ...
        missing_css = family_source_codepoints - css_codepoints(family_dir / "HanaMin.css")
        if missing_css:
    ...
    checks = {
        ...
        "source variation sequences": (len(source_sequences), EXPECTED_VARIATION_SEQUENCES),
        "output variation sequences": (len(output_sequences), EXPECTED_VARIATION_SEQUENCES),
    ...
    if failed:
        raise SystemExit("Hanamin audit failed")
