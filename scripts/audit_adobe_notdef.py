...
SOURCE = ROOT / ".work/adobe-notdef/source/AND-Regular.otf"
DIST = ROOT / "packages/adobe-notdef/dist"
...
def main() -> None:
    ...
    if not output_paths:
        raise SystemExit("no Adobe NotDef WOFF2 files found")
    output_points = set().union(*(codepoints(path) for path in output_paths))
    css_points = css_codepoints(DIST / "Adobe-NotDef.css")
    ...
    if not source_points == output_points == css_points:
        raise SystemExit("Adobe NotDef codepoint sets differ")
