...
- **Search:** Search indexes the catalog label, every localized upstream official name, package identifier, classification, language, and description. Fuzzy matches tolerate minor typing errors and rank official names and catalog labels ahead of supporting metadata. An empty query preserves registry order. Search remains debounced so result rendering does not compete with typing.
...
- **Style:** Each font is a full-width ruled section, not a card. Its header keeps the catalog label, locale-matched upstream official name when distinct, exact package version, coverage state, and variant selection together.
...
- **Style:** Labeled HTML stylesheet and CSS `font-family` examples each place copyable code in the fluid cell and a high-contrast action in a fixed-width cell. The CSS declaration preserves the active variant's published family sequence and ends in `serif`, `sans-serif`, `monospace`, or `cursive` according to its classification.
